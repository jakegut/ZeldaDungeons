...
public class DungeonRoom {
    ...
    public Point getSpawnPoint() {
        return new Point(offsetX + 6 * GameDungeon.TILE_SIZE,
                offsetY + 6 * GameDungeon.TILE_SIZE);
    }
    ...
    public void setUpBox2d(World world) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                float tileSize = tile(x, y).isDoor() ? GameDungeon.TILE_SIZE - 2 : GameDungeon.TILE_SIZE;
                Body b = Box2DUtils.createStaticRectBody(world, realX, realY, tileSize, tileSize);
